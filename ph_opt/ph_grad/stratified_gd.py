...
try:
    sys.path.append('../multipers/')
    from dionysus_vineyards import ls_vineyards as lsvine
except ImportError:
    print('vineyards not available. Set use_vineyards to False.')
...
def STPers_V(fct, simplex, dim, card_strata):
    '''
    Simplex Tree tensorflow compatible persistence for
    VINEYARDS
    '''
    n = int(len(fct)/(2*card_strata))
    V = lsvine(np.reshape(fct[:card_strata*n], [card_strata,-1]), simplex, 1)[dim]
    P = np.array(np.reshape(fct[card_strata*n:2*card_strata*n], [card_strata,-1]), dtype=np.int32)
    V = np.vstack([np.reshape(v,[-1,3]) for v in V])
    idxdg = V[:,2]
    VS = [V[np.argwhere(idxdg==c).ravel()][:,:2] for c in range(card_strata)]

    VVS = []
    cdg = 0
    for ndg in range(card_strata):
        PP = P[ndg,:]
        di = PP[np.array(VS[ndg], dtype=np.int32)]
        indices = di.flatten()
        VVS.append(indices)
        cdg = max(cdg,len(di))
    indices = np.concatenate([np.array([cdg])] + [np.pad(ix[:min(len(ix),2*cdg)],((0,max(0,2*cdg-len(ix))))) for ix in VVS])
    return indices
...
class TopoMeanModel(tf.keras.Model):
    ...
    def loss(self, STPersTF, epoch):
        ...
        if self.mode == 'vanilla' or self.mode == 'gradient_sampling':
            ...
            # Removed juste a bit coz it was printing too much.
    ...
    def call(self, epoch):
        ...
        if self.use_vineyards:
            STPersTF = lambda fct: STPers_V(fct, self.simplex, self.dim, self.card_strata)
        elif self.extended:
...
def _reduce_gradient(topomean,
    ...
    if mode == 'strata':
        ...
        while True:
            # Instead of checking if the loss decreases explicitly (which requires computations),
            # we check whether we can show that it decreases using epsilon condition, and if this condition 
            # is not met, we make it true. In particular, this means that we will always reduce epsilon 
            # if the condition is not met, even if the loss was actually decreasing, making this code
            # a bit slower than the algorithm in the paper (since epsilon is smaller than the theory). 
            if good_epsilon <= constant*good_norm_grad or good_norm_grad <= eta:
                break
            else:
                epstimes += 1
                # loop_epsilon allows to progressively reduce epsilon instead of "brute force" update
                # Somehow it is a mix between SimpleUpdateStep (since lipschitz constant L is known) 
                # and UpdateStep (since there is a parameter gamma for progressive reduction) 
                if loop_epsilon:  
                    # reduce epsilon
                    good_epsilon *= gamma
                ...
                else: # this corresponds exactly to SimpleUpdateStep
        ...
        # In gradient sampling mode, epsilon corresponds to t_k in the paper 
        # to avoid creating new variables for TopoMeanModel
        good_single_grad = single_grad
        good_norm_grad = norm_grad
        good_epsilon = epsilon
...
def compute_gradient(epoch, dgms, gradients, funcs, topomean, loop_epsilon=True):
    ...
    if topomean.mode == 'gradient_sampling':
        ...
        if norm_grad < topomean.eta:
            ...
            good_epsilon,_,_ = _reduce_gradient(topomean,
                                                grads, single_grad, norm_grad,
